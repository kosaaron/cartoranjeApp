...
if ($no_of_row == 0) {
	...
	$insert_query = "
	UPDATE users
	SET UserActCode = :user_activation_code
    WHERE Email = :user_email
	";
	...
	if ($host == "localhost") {
		$testlink = 'localhost/cartoranjeAPP/login.php?act_code=' . $user_activation_code . '&new_pass=true';
		...
		$baseURL = "https://$host/login.php?act_code=$user_activation_code&new_pass=true";
		...
		$mail->Subject = 'Jelszó módosítás';			//Sets the Subject of the message
		$mail->Body = $mail_body;							//An HTML or plain text message body

		if ($mail->Send())								//Send an Email. Return true on success or false on error
		{
			$main_data['EmailSent'] = TRUE;
			$main_data['Message'] = 'Email elküldve, nézd meg a bejövő üzeneteidet!';
		}
	}
}
